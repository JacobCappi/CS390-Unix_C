...
/* -----
// Note: I thought about moving and organizing, but functions were
//       too dependent on local variables.
// Sorry: I still tried to organize as best I could
// ----- */

/* do exec runs execv(..) */
/* added as Program 2 */
int do_exec(int argcount, char **args);
...
/* ----- END */
    ...
    /* ----- START Logic: */
        /* ----- START mysh2.c specific code: do_exec */
        ...
        /* ----- START echo */
        else if(strcmp(command[0], commandList[0]) == 0){
            i=1;
            flag = 0;
...
/* spent 2 hours figuring out why access didn't work. dangling new line is stupid */
    if(access(path, F_OK) != 0){
        path[strlen(path)-1] = 0;
        if(access(path, F_OK) != 0){
            printf("shell: %sNo such file or directory\n", path);
            return -1;
        }
        if(access(path, X_OK) != 0){
            printf("shell: %sFile permission denied\n", path);
            return -1;
        }
    }
    if(argcount == 2){
